...
OUTPUT_PATH = "outputs/intro.mp4"
FINAL_SIZE = (1920, 1080)  # Full HD 16:9

# --- Tuned PIP motion ---
START_SCALE = 0.9
END_SCALE = 0.3
X_END_RATIO = 0.12          # move further left
Y_END_RATIO = 0.82          # same vertical margin
MOVE_PORTION = 0.4
CAPTION_BOTTOM_OFFSET = 220 # lift caption more
# ----------------------------


def create_caption(text, width, height, duration, delay, fontsize=42):
    ...
    clip = clip.set_start(delay)  # start after the audio delay
...
scene = script["scenes"][0]  # welcome scene
...
# === Load audio ===
audio_path = os.path.join(AUDIO_DIR, "scene_001.wav")
...
def pos_at_time(t):
    ...
    x_end, y_end = W * X_END_RATIO, H * Y_END_RATIO
    progress = min(t / (TOTAL_DURATION * MOVE_PORTION), 1)
    eased = ease_out_cubic(progress)
...
print(f"✅ Final intro exported: {OUTPUT_PATH}")
